...
const subscribeComments = (platform, observeElement, sendResponse) => {
  ...
  const observer = new MutationObserver(async function (records) {
    await sendToBackground({name: 'hoge', body: {command: 'Subscribe', comments: extractComment(records)}})
...
> = async (req, res) => {
  ...
  const observeElement = document.querySelector<HTMLDivElement>(selectors['slack'].listNodeSelector);
    // zoom
  ...
  await sendToBackground({name: 'hoge', body: {command: 'Load', from: 'Subscriber', tabId: req.tabId}})
  // res.send(hoge)
}
