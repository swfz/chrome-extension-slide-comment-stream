...
const connect = async (feature: Feature, role: Role, service: Service, tabId: number) => {
  const storage = new Storage({ area: "local" });
  const state = (await storage.get<ConnectedStatus>(STORAGE_KEY)) || {};
  const config = (await storage.get<Config>("config")) || ({} as Config);
  const identifier = `${feature}_${role}`;
...
const handler: PlasmoMessaging.MessageHandler<ConnectRequestBody, WorkerResponseBody> = async (
  req,
  res,
) => {
