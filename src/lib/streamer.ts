...
const removelinkBar = (iframeElement: HTMLIFrameElement) => {
  const linkBar = iframeElement.contentWindow?.document.querySelector<HTMLDivElement>(
  ...
  const wrapper = iframeElement.contentWindow?.document.querySelector<HTMLDivElement>(
...
const renderClap = (boxElement: HTMLDivElement, config: Config, claps: number): void => {
...
const renderComment = (boxElement: HTMLDivElement, config: Config, comment: string) => {
...
const render = (boxElement: HTMLDivElement, config: Config, comments: string[]) => {
  const claps = comments.reduce((n, c) => n + (c.match(/[8８]/g)?.length ?? 0), 0);
  if (claps > 0) {
